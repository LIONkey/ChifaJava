...
public class Salida {
    ...
    private final String SQL_INSERT_SALIDA = "INSERT INTO tblSalida (sal_factura, sal_pro_codigo, sal_fecha, sal_cantidad) VALUES (?,?,?,?)";
    private final String SQL_SELECT_SALIDA = "SELECT sal_factura, sal_fecha, sal_pro_codigo, pro_Descripcion, sal_cantidad FROM tblSalida "
            + "INNER JOIN tblProducto ON tblSalida.sal_pro_codigo = tblProducto.pro_Codigo";
    ...
    public int verificarStock(String codigo) {
        int res = 0;
        try {
            PS = CN.getConnection().prepareStatement("SELECT inv_stock from tblInventario where inv_pro_codigo='" + codigo + "'");
            RS = PS.executeQuery();
